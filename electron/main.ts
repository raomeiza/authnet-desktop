...
import { IStaff, IStudent, ICourse, IDepartment, IFaculty, ISchool, IStudentAttendance } from './interfaces';
...
let fingerprintLoginIn: null | true | false = null;
...
let scanner: { inUse: boolean, window: IWindow | null, timestamp: number | null } = {
  inUse: false,
  window: null,
  timestamp: null
}
...
let fingerPrintEnrollingUser: { id: string, fid1: number | null, fid2: number | null, preventDuplicate?: boolean } | null = null;

// Function to create the browser window
async function createWindow() {
  database = new Database();
  const staffCount = await database.staff().count();
  ...
  mainWindow.loadURL('http://localhost:' + serverPort + '?count=' + staffCount); // Assuming React dev server is running on serverPort 3000
...
async function findESP32Port() {
  ...
  let esp32Port: string | undefined;

  for (const portInfo of ports) {
    let testPort: SerialPort | undefined;
    try {
      // Try to open a connection on each port
      testPort = new SerialPort({ path: portInfo.path, baudRate: 57600, autoOpen: false });
          ...
          testPort!.write('details\n', (writeErr) => {
            ...
            testPort!.once('data', (data) => {
              try {
                ...
                if (response.result?.capacity) { // Adjust this condition based on the expected response
...
class LineTransform extends Transform {
  ...
  _transform(chunk: { toString: () => string; }, encoding: any, callback: () => void) {
...
async function setupSerialConnection() {
  try {
    const esp32Port = await findESP32Port();
    ...
    //@ts-ignore
    port = new SerialPort({ path: esp32Port, baudRate: 57600 });
    //@ts-ignore
    ...
    parser.on('data', async (data: { toString: () => string; }) => {
      try {
          ...
          if (jsonData.command === 'enroll' && jsonData.event === 'result' && fingerPrintEnrollingUser) {
            ...
            if (fingerPrintEnrollingUser.fid1 === null) {
              ...
              if (fingerPrintEnrollingUser.fid1 !== null && fingerPrintEnrollingUser.fid2 !== null) {
                try {
                  const result = await database.fingerprint().enroll(fingerPrintEnrollingUser.id, fingerPrintEnrollingUser.fid1, fingerPrintEnrollingUser.fid2);
                    ...
                    try {
                      ...
                      if( fingerPrintEnrollingUser.preventDuplicate) {
                        let user = fingerPrintEnrollingUser.id.includes('@') ?
                          await database.staff().findByEmail(fingerPrintEnrollingUser.id) :
                          await database.student().findByMatricNumber(fingerPrintEnrollingUser.id);
                        //@ts-ignore
                        if (user?.email === fingerPrintEnrollingUser.id || user?.matric === fingerPrintEnrollingUser.id) {
                        ...
                        const removed = await database.fingerprint().remove(fingerPrintEnrollingUser.id);
                        if (removed) {
            ...
            if(jsonData.result?.score && jsonData.result?.score < 75) {
              if (scanner.inUse && scanner.window) {
            ...
            if (jsonData.command === 'search' && fingerprintLoginIn) {
              ...
              if (jsonData.result && (jsonData.result.id || jsonData.result.id === 0)) { // check if the result is a valid fingerprint id
                ...
                try {
                  const result = await database.staff().findByFid(jsonData.result.id);
                  if (result) {
                    user = result;
              ...
              return;
            }else if (jsonData.command === 'search' && fingerPrintTakingAttendance.inUse && fingerPrintTakingAttendance.courseCode?.endsWith('_EXAM')) {
  if (jsonData.result && jsonData.result.id) {
    try {
      const student = await database.student().findByFid(jsonData.result.id);
      if (student) {
        ...
        const courseStudents = JSON.parse(fingerPrintTakingAttendance.course?.students || '[]');
        if (!courseStudents.map((s: string) => s.trim()).includes(student.matric.trim())) {
        ...
        const allAttendance = await database.studentAttendance().getByCourseCode(actualCourseCode);
        
        // Group attendance by date to count total classes
        const classesByDate = allAttendance.reduce((acc: any, attendance: IStudentAttendance) => {
        ...
        const studentAttendance = allAttendance.filter((attendance: IStudentAttendance) => 
          attendance.studentMatric === student.matric
        );
              ...
              if (jsonData.result && jsonData.result.id) {
                ...
                try {
                  ...
                  if (student) {
                    ...
                    const existingAttendance = await database.studentAttendance().getByCourseCodeAndDate(
                      fingerPrintTakingAttendance.courseCode!, 
                      fingerPrintTakingAttendance.date!
                    );
                    
                    const alreadyMarked = existingAttendance.some((attendance: IStudentAttendance) => 
                      attendance.studentMatric === student.matric
                    );
                    ...
                    const attendanceRecord: IStudentAttendance = {
                    ...
                    const result = await database.studentAttendance().markAttendance(attendanceRecord);
...
ipcMain.on('serial-send', (event: { sender: { send: (arg0: string, arg1: { event: string; message: string; error: { message: string; } | { message: string; }; }) => void; }; }, command: string, commandId: any, preventDuplicate: boolean = true) => {
  if (scanner.inUse) {
...
ipcMain.on('create-new-window', (event: any, { url, width, height, title }: any) => {
...
ipcMain.on('close-window', (event: { sender: { close: () => void; }; }, url?: string) => {
  if (url === undefined) {
...
ipcMain.on('enroll-student', async (event: { sender: { send: (arg0: string, arg1: { error: string; }) => void; }; reply: (arg0: string, arg1: { result?: IStudent; error?: unknown; }) => void; }, student: IStudent) => {
  let stuFid = fingerPrintEnrollingUser
  if (!stuFid?.id) {
    try {
      stuFid = await database.fingerprint().find(student.matric);
    } catch (err: any) {
      // send error message to the window that sent the command
  ...
  if (stuFid?.fid1 === null || stuFid?.fid2 === null) {
    // send error message to the window that sent the command
  ...
  try {
    ...
    const result = await database.student().enroll(student); // Example function
    ...
    // remove the old data from the fingerprint table
    await database.fingerprint().remove(student.matric);
  } catch (error) {
...
ipcMain.on('enroll-staff', async (event: { reply: (arg0: string, arg1: { error?: any; message?: string; result?: IStaff; }) => void; }, staff: IStaff) => {
  let staffFID = fingerPrintEnrollingUser
  if (!staffFID?.id) {
    try {
      staffFID = await database.fingerprint().find(staff.email);
    } catch (err: any) {
      // send error message to the window that sent the command
      event.reply('enroll-staff-response', { error: err });
    }
  }
  if (staffFID?.fid1 === null || staffFID?.fid2 === null) {
    // send error message to the window that sent the command
    event.reply('enroll-staff-response', { message: 'Fingerprint not found' });
    // remove the old data from the fingerprint table
    const removed = await database.fingerprint().remove(staff.email);
    return;
  }
  if (staffFID) {
  ...
  try {
    // Perform the enrollment logic here
    const result = await database.staff().enroll(staff);
    // remove the old data from the fingerprint table
    const removed = await database.fingerprint().remove(staff.email);
...
ipcMain.on('create-course', async (event: { reply: (arg0: string, arg1: { result?: ICourse; error?: unknown; }) => void; }, course: ICourse) => {
  try {
    course.createdBy = user.email;
    const result = await database.course().create(course);
...
ipcMain.on('get-courses', async (event: { reply: (arg0: string, arg1: { result?: ICourse[]; error?: unknown; }) => void; }) => {
  try {
    const result = await database.course().getAll();
...
ipcMain.on('get-students', async (event: { reply: (arg0: string, arg1: { result?: IStudent[]; error?: unknown; }) => void; }) => {
  try {
    const result = await database.student().getAll();
...
ipcMain.on('get-staff', async (event: { reply: (arg0: string, arg1: { result?: IStaff[]; error?: unknown; }) => void; }) => {
  try {
    const result = await database.staff().getAll();
...
ipcMain.on('staff-login', async (event: { reply: (arg0: string, arg1: { error?: unknown; result?: IStaff; }) => void; }, payload: { email: string, password: string }, url: string) => {
  try {
    const result = await database.staff().login(payload.email, payload.password);
...
ipcMain.on('staff-fingerprint-login', async (event: { sender: any; reply: (arg0: string, arg1: { error: unknown; }) => void; }) => {
  try {
...
ipcMain.on('get-scanner-state', (event: { reply: (arg0: string, arg1: { result: { connected: boolean; inUse: boolean; }; }) => void; }) => {
  event.reply('scanner-state-response', { result: { connected, inUse: scanner.inUse } });
});

// create a listener to get the count of staffs
ipcMain.on('get-staff-count', async (event: { reply: (arg0: string, arg1: { result?: number; error?: unknown; }) => void; }) => {
  try {
    const result = await database.staff().count();
...
ipcMain.on('get-students-from-level', async (event: { reply: (arg0: string, arg1: { result?: IStudent[]; error?: unknown; }) => void; }, level: string) => {
  try {
    let result = await database.student().getByLevel(level);
...
ipcMain.on('get-course-attendance', async (event: { reply: (arg0: string, arg1: { result?: IStudentAttendance[]; error?: unknown; }) => void; }, course: string) => {
  try {
    let result = await database.studentAttendance().getByCourseCode(course);
...
ipcMain.on('get-student-by-matric', async (event: { reply: (arg0: string, arg1: { result?: IStudent; error?: unknown; }) => void; }, matric: string) => {
  try {
    let result = await database.student().findByMatricNumber(matric);
...
ipcMain.on('get-course', async (event: { reply: (arg0: string, arg1: { result?: ICourse; error?: unknown; }) => void; }, courseCode: string) => {
  try {
    let result = await database.course().getByCourseCode(courseCode);
...
ipcMain.on('get-courses-by-level', async (event: { reply: (arg0: string, arg1: { result?: ICourse[]; error?: unknown; }) => void; }, level: string) => {
  try {
    let result = await database.course().getByLevel(level);
...
ipcMain.on('mark-attendance', async (event: { reply: (arg0: string, arg1: { error: unknown; }) => void; }, courseCode: string, date?: string,) => {
  try {
    ...
    let course = await database.course().getByCourseCode(courseCode);
...
ipcMain.on('stop-marking-attendance', async (event: { reply: (arg0: string, arg1: { message?: string; error?: unknown; }) => void; }) => {
  try {
    const wasExamAttendance = fingerPrintTakingAttendance.courseCode?.endsWith('_EXAM');
...
ipcMain.on('update-course-students', async (event: { reply: (arg0: string, arg1: { result?: string; error?: unknown; }) => void; }, courseCode: string, students: string) => {
  try {
    let result = await database.course().updateStudents(courseCode, students);
...
ipcMain.on('update-course', async (event: { reply: (arg0: string, arg1: { result?: ICourse; error?: unknown; }) => void; }, courseCode: string, courseData: Partial<ICourse>) => {
  try {
    let result = await database.course().updateCourse(courseCode, courseData);
...
ipcMain.on('delete-course', async (event: { reply: (arg0: string, arg1: { result?: string; error?: unknown; }) => void; }, courseCode: string) => {
  try {
    let result = await database.course().remove(courseCode);
...
ipcMain.on('export-students-to-csv', async (event: { reply: (arg0: string, arg1: { success?: boolean; error?: unknown; }) => void; }) => {
  try {
    let csv = await database.student().exportToCSV();
...
ipcMain.on('export-courses-to-csv', async (event: { reply: (arg0: string, arg1: { success?: boolean; error?: unknown; }) => void; }) => {
  try {
    let csv = await database.course().exportToCSV();
...
ipcMain.on('export-attendance-to-csv', async (event: { reply: (arg0: string, arg1: { success?: boolean; error?: unknown; }) => void; }) => {
  try {
    let csv = await database.studentAttendance().exportToCSV();
...
ipcMain.on('get-staffs', async (event: { reply: (arg0: string, arg1: { result?: IStaff[]; error?: unknown; }) => void; }) => {
  try {
    const result = await database.staff().getAll();
...
ipcMain.on('mark-manual-attendance', async (event: { reply: (arg0: string, arg1: { result?: IStudentAttendance; error?: unknown; }) => void; }, studentMatric: string, courseCode: string, date?: string) => {
  try {
    date = date || new Date().toISOString().split('T')[0]; // YYYY-MM-DD format
    
    // Check if student exists
    const student = await database.student().findByMatricNumber(studentMatric);
    if (!student) {
    ...
    const course = await database.course().getByCourseCode(courseCode);
    if (!course) {
    ...
    const existingAttendance = await database.studentAttendance().getByCourseCodeAndDate(courseCode, date);
    const alreadyMarked = existingAttendance.some((attendance: IStudentAttendance) => 
      attendance.studentMatric === studentMatric
    );
...
ipcMain.on('get-attendance-stats', async (event: { reply: (arg0: string, arg1: { result?: {total: number, present: number, absent: number}; error?: unknown; }) => void; }, courseCode: string, date?: string) => {
  try {
    const result = await database.studentAttendance().getAttendanceStats(courseCode, date);
...
ipcMain.on('get-student-attendance', async (event: { reply: (arg0: string, arg1: { result?: IStudentAttendance[]; error?: unknown; }) => void; }, studentMatric: string, courseCode?: string) => {
  try {
    let result: IStudentAttendance[];
    if (courseCode) {
      result = await database.studentAttendance().getByStudentMatricAndCourse(studentMatric, courseCode);
    } else {
      result = await database.studentAttendance().getByStudentMatric(studentMatric);
    }
...
ipcMain.on('delete-student', async (event: { reply: (arg0: string, arg1: { result?: boolean; error?: unknown; }) => void; }, studentMatric: string) => {
  try {
    const res = await database.student().deleteByMatric(studentMatric);
...
ipcMain.on('delete-staff', async (event: { reply: (arg0: string, arg1: { result?: boolean; error?: unknown; }) => void; }, staffId: number) => {
  try {
    const result = await database.staff().deleteById(staffId);
...
ipcMain.on('update-student', async (event: { reply: (arg0: string, arg1: { result?: IStudent; error?: unknown; }) => void; }, studentMatric: string, studentData: IStudent) => {
  let stuFid = fingerPrintEnrollingUser
  if(!studentData.fid1 && !studentData.fid2) {
    if (!stuFid?.id) {
    try {
      stuFid = await database.fingerprint().find(studentData.matric);
    } catch (err: any) {
      // send error message to the window that sent the command
  ...
  try {
    const result = await database.student().updateStudentData(studentMatric, studentData);
    // delete the old fingerprint data
    await database.fingerprint().remove(studentMatric);
...
ipcMain.on('update-staff', async (event: { reply: (arg0: string, arg1: { result?: IStaff; error?: unknown; }) => void; }, staffEmail: string, staffData: IStaff) => {
  let staffFID = fingerPrintEnrollingUser
  if (!staffData.fid1 && !staffData.fid2) {
    if (!staffFID?.id) {
      try {
        staffFID = await database.fingerprint().find(staffData.email as string);
      } catch (err: any) {
        // send error message to the window that sent the command
        event.reply('serial-data', { error: 'Error fetching fingerprint data' });
        console.error('Failed to find fingerprint:', err);
  ...
  try {
    const result = await database.staff().updateStaffData(staffEmail, staffData);
    // delete the old fingerprint data
    await database.fingerprint().remove(staffEmail);
...
ipcMain.on('delete-student-fids', async (event: { reply: (arg0: string, arg1: { result?: boolean; error?: unknown; }) => void; }, studentMatric: string) => {
  try {
    ...
    if (student.fid1 !== null && student.fid1 !== undefined) {
      deletionPromises.push(new Promise((resolve, reject) => {
        ...
        const dataHandler = (data: any) => {
          try {
            const response = JSON.parse(data.toString().trim());
            if (response.command === 'delete' && response.result?.id === student.fid1) {
    ...
    if (student.fid2 !== null && student.fid2 !== undefined) {
      deletionPromises.push(new Promise((resolve, reject) => {
        ...
        const dataHandler = (data: any) => {
          try {
            const response = JSON.parse(data.toString().trim());
            if (response.command === 'delete' && response.result?.id === student.fid2) {
              clearTimeout(timeout);
              port.removeListener('data', dataHandler);
    ...
    const result = await database.student().clearFids(studentMatric);
...
ipcMain.on('delete-staff-fids', async (event: { reply: (arg0: string, arg1: { result?: boolean; error?: unknown; }) => void; }, staffEmail: string) => {
  try {
    // First, get the staff data to retrieve their FIDs
    const staff = await database.staff().findByEmail(staffEmail);
    if (!staff) {
    ...
    if (staff.fid1 !== null && staff.fid1 !== undefined) {
      deletionPromises.push(new Promise((resolve, reject) => {
        ...
        const dataHandler = (data: any) => {
          try {
            const response = JSON.parse(data.toString().trim());
            if (response.command === 'delete' && response.result?.id === staff.fid1) {
              clearTimeout(timeout);
              port.removeListener('data', dataHandler);
    ...
    if (staff.fid2 !== null && staff.fid2 !== undefined) {
      deletionPromises.push(new Promise((resolve, reject) => {
        ...
        const dataHandler = (data: any) => {
          try {
            const response = JSON.parse(data.toString().trim());
            if (response.command === 'delete' && response.result?.id === staff.fid2) {
              clearTimeout(timeout);
              port.removeListener('data', dataHandler);
    ...
    // Clear the FIDs from the database regardless of scanner deletion success
    const result = await database.staff().clearFids(staffEmail);
...
ipcMain.on('clear-fingerprint-scanner', async (event: { reply: (arg0: string, arg1: { result?: boolean; error?: unknown; }) => void; }, password: string) => {
  try {
    if (!password) {
    ...
    const dataHandler = (data: { toString: () => string; }) => {
...
ipcMain.on('delete-fid', async (event: { reply: (arg0: string, arg1: { result?: boolean; error?: unknown; }) => void; }, fid: string) => {
    try {
        if (!connected) {
...
ipcMain.on('get-staff-by-id', async(event: { reply: (arg0: string, arg1: { result?: any; error?: unknown; }) => void; }, staffId: number) => {
  try {
    const staff = await database.staff().findById(staffId);
    if (!staff) {
...
ipcMain.on('export-database', async (event: { reply: (arg0: string, arg1: { success?: boolean; filePath?: string; error?: unknown; }) => void; }) => {
  try {
    ...
    if (filePath) {
      ...
      const exportedFilePath = await database.ldb().exportToFile(filePath);
...
ipcMain.on('import-database', async (event: { reply: (arg0: string, arg1: { success?: boolean; error?: unknown; }) => void; }) => {
  try {
    ...
    if (filePaths && filePaths.length > 0) {
      ...
      if (confirmResult.response === 1) { // User clicked Import
        ...
        const result = await database.ldb().importFromFile(filePath);
...
ipcMain.on('clear-database', async (event: { reply: (arg0: string, arg1: { success?: boolean; error?: unknown; }) => void; }, password: string) => {
  try {
    if (!password) {
    ...
    // Verify password
    const verified = await database.staff().verifyPassword(user.email, password);
    if (!verified) {
    ...
    if (confirmResult.response === 1) { // User clicked Clear Database
      // Clear the database
      const result = await database.ldb().clear();
...
ipcMain.on('get-database-stats', async (event: { reply: (arg0: string, arg1: { result?: any; error?: unknown; }) => void; }) => {
  try {
    const stats = {
      students: await database.student().count(),
      staff: await database.staff().count(),
      courses: await database.course().count(),
      attendanceRecords: (await database.studentAttendance().get()).length,
    };
...
ipcMain.on('backup-database', async (event: { reply: (arg0: string, arg1: { success?: boolean; filePath?: string; error?: unknown; }) => void; }) => {
  try {
...
ipcMain.on('restore-database', async (event: { reply: (arg0: string, arg1: { success?: boolean; error?: unknown; }) => void; }, password: string) => {
  try {
    if (!password) {
...
// create a listener for marking exam attendance
ipcMain.on('mark-exam-attendance', async (event: { reply: (arg0: string, arg1: { result?: any; error?: unknown; }) => void; }, courseCode: string, date?: string) => {
  try {
    if (fingerPrintTakingAttendance.inUse) {
