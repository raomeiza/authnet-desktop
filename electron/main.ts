...
function loadErrorPage(window: BrowserWindow, failedUrl: string, errorMessage: string) {
...
async function injectElectronCookie(window: BrowserWindow, url: string) {
  try {
...
ipcMain.on('create-new-window', async (event: any, { url, width, height, title }: any) => {
...
ipcMain.handle('probe-openwrt', async () => {
  try {
    ...
    const hasLuciHeaders = responseHeaders['x-luci-login-required'] === 'yes' ||
                          responseHeaders['server']?.toLowerCase().includes('luci') ||
    ...
    if (hasLuciHeaders) {
      ...
      if (hasAntiIndicators) {
        ...
        return {
          ...
          details: { 
            networkCheck,
            antiIndicators: antiIndicators.filter(indicator => 
              htmlContent.toLowerCase().includes(indicator.toLowerCase())
            ),
      ...
      foundIndicators = openwrtIndicators.filter(indicator => 
        htmlContent.toLowerCase().includes(indicator.toLowerCase())
      );
...
async function checkDirectOpenWrtConnection(): Promise<{
...
ipcMain.handle('check-internet-connectivity', async () => {
  try {
    ...
    for (const target of targets) {
      try {
        const { stdout, stderr } = await execAsync(`nslookup ${target}`, { timeout: 5000 });
        ...
        results.push({
          target,
          success: false,
          output: (error as Error).message || 'Command failed'
        });
      }
    }
...
ipcMain.handle('ssh-get-status', async () => {
  return {
    ...sshConnectionStatus,
    timestamp: new Date().toISOString()
  };
...
ipcMain.on('retry-connection', async (event, url: string) => {
...
async function checkAndResolveIpConflict(): Promise<{ success: boolean; error?: string; output?: string }> {
  const reconnectTimeout = 5000;
  const reconnectRetries = 3;
  try {
    // If device is already on an adopted 172.31 network, skip conflict resolution.
    try {
      const probe = await checkDirectOpenWrtConnection();
      const gw = probe?.details?.gateway || null;
    ...
    const conn = await establishSSHConnection();
    if (!conn.success) {
    ...
    const result = await executeSSHCommand(script, 30000);
    ...
    if (output.includes('Applying network changes') || output.includes('restart')) {
      for (let i = 0; i < reconnectRetries; i++) {
        await new Promise((r) => setTimeout(r, reconnectTimeout));
        const reConn = await establishSSHConnection();
        if (reConn.success) {
...
async function apiCall(endpoint: string, authToken: string, body: any = {}) {
...
ipcMain.handle('start-automated-deployment', async (event, { authToken, businessId, wifiName, realmId }) => {
  ...
  try {
    ...
    try {
      const ipCheckResult = await checkAndResolveIpConflict();
      ...
      return { success: false, error: `IP conflict resolution failed: ${err?.message || err}` };
    }

    

    // Initialize deployment session with API server
    const initPayload: any = { businessId, realmId, wifiName: wifiName?.trim() };
...
async function executeNextDeploymentStep() {
  ...
  try {
    ...
    const stepResult = await apiCall('/onboard/get-next-step', authToken, {
      sessionId,
      currentStepId: isFirstStep ? null : stepSummaries[currentStepIndex - 1]?.id,
      routerInfo: null
    });
    ...
    const commandResult = await executeSSHCommand(step.command, step.timeout || 300000);
    ...
    const validationResult = await apiCall('/onboard/execute-step', authToken, {
      sessionId,
      stepId: step.id,
      testResult: {
...
// Stop deployment
ipcMain.handle('stop-deployment', async () => {
  if (deploymentState.isRunning) {
    deploymentState.isRunning = false;
    deploymentState.isPaused = false;
...
ipcMain.handle('retry-deployment-step', async () => {
...
// Initialize deployment session with API server
ipcMain.handle('api-initialize-deployment', async (event, { authToken, businessId }) => {
  console.warn('api-initialize-deployment is deprecated. Use start-automated-deployment instead.');
  return await apiCall('/onboard/initialize', authToken, { businessId });
});

// Get next step from API server
ipcMain.handle('api-get-next-step', async (event, { authToken, sessionId, currentStepId, routerInfo }) => {
  console.warn('api-get-next-step is deprecated. Deployment steps are now handled automatically.');
  return await apiCall('/onboard/get-next-step', authToken, { sessionId, currentStepId, routerInfo });
});

// Submit step execution result to API server
ipcMain.handle('api-execute-step', async (event, { authToken, sessionId, stepId, testResult }) => {
  console.warn('api-execute-step is deprecated. Deployment steps are now handled automatically.');
  return await apiCall('/onboard/execute-step', authToken, { sessionId, stepId, testResult });
});
